...
void* thread_fn(void* arg){
    ...
    if(ss_socket == -1){
        perror("Error creating ss_socket to send ALIVE packets");
        exit(EXIT_FAILURE);
    }
    ...
    if(connect(ss_socket, (struct sockaddr*)&server, sizeof(server)) == -1){
        perror("Error connecting ss_socket to nm to send ALIVE packets");
        exit(EXIT_FAILURE);
    }

    while(1){
        char* alive_packet = "ALIVE";
        if(send(ss_socket, alive_packet, strlen(alive_packet), 0) == -1){
            perror("Error sending ALIVE packet to nm");
            close(ss_socket);
            exit(EXIT_FAILURE);
        }
        sleep(4);
...
int main(int argc, char *argv[])
{
    ...
    char ack[50];
    recv(clientsock, &ack, sizeof(ack), 0);

    registered_servers[num_registered_servers] = ss_i;
    num_registered_servers++;

    if(pthread_create(&thread, NULL, thread_fn, &port) != 0){
        perror("Failed to create thread to send ALIVE packets to nm");
        exit(EXIT_FAILURE);
    }
    //}
