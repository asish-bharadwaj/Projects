...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      if (p->state == RUNNABLE){
        if(fcp == 0 || p->ctime < fcp->ctime)
          fcp = p;
      }
    }
    if(fcp != 0){
      acquire(&fcp->lock);
      if(fcp->state == RUNNABLE){
        fcp->state = RUNNING;
        c->proc = fcp;
        swtch(&c->context, &fcp->context);
        c->proc = 0;
      }
      release(&fcp->lock);
    }
      ...
      for(int i = 0; i < pq[0]; i++){
        ...
        while(p->state == RUNNABLE && p->clk_int < 1){
          ...
          if(p->state == RUNNABLE){
              ...
              if(proc[j].pid != p->pid && proc[j].state == RUNNABLE){
                ...
                if(proc[j].vol_exit == 1){
                  proc[j].vol_exit = 0;
                    ...
                    // printf("pid - %d priority_until_now - %d, changed to - %d ticks - %d\n", proc[j].pid, -1, proc[j].priority, ticks);
                  ...
                  if(proc[j].wait_time > 50 && proc[j].priority >= 1){
                    ...
                    // printf("pid - %d priority_until_now - %d, changed to - %d ticks - %d\n", proc[j].pid, proc[j].priority, proc[j].priority-1, ticks);
                    pq[proc[j].priority]--;
                    proc[j].priority--;
                    PQ[proc[j].priority][pq[proc[j].priority]++] = &proc[j];
                  }
            ...
            // printf("pid - %d priority_until_now - %d, changed to - %d ticks - %d\n", p->pid, p->priority, -1, ticks);
            pq[p->priority]--;
            // printf("vol exit handled\n");
          }
        }
        if(p->clk_int == 1){
          // printf("ran\n");
          ...
          pq[p->priority]--;
                    // printf("pid - %d priority_until_now - %d, changed to - %d ticks - %d\n", p->pid, p->priority, p->priority+1, ticks);
          p->priority++;
              ...
              if(proc[j].pid != p->pid && proc[j].state == RUNNABLE){
                ...
                if(proc[j].vol_exit == 1){
                  proc[j].vol_exit = 0;
                    ...
                    // printf("pid - %d priority_until_now - %d, changed to - %d ticks - %d\n", proc[j].pid, proc[j].priority, -1, ticks);
                  PQ[proc[j].priority][pq[proc[j].priority]++] = &proc[j];
                }
                else{
                  proc[j].wait_time++;
